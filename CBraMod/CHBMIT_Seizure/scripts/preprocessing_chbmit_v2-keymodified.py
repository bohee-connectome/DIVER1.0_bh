#!/usr/bin/env python3
"""
CHB-MIT Seizure Detection Dataset Preprocessing for DIVER (v2-keymodified)

VERSION: v2-keymodified
- Modified to match ISRUC-Sleep data dictionary format
- Changed 'signal' -> 'sample', 'metadata' -> 'data_info'
- Added ISRUC-compatible metadata fields (Dataset, modality, task, etc.)
- Preserved CHBMIT-specific fields (segment_id, is_oversampled, split)

Original version backed up as: preprocessing_chbmit_v1_original.py
...
Author: Claude + User
Date: 2025-01-21
Modified: 2025-01-27 (v2-keymodified)
"""
            ...
            for split in ['train', 'val', 'test']:
                if split in result and len(result[split]) > 0:
                    ...
                    for seg in segments:
                        ...
                        # Modified to match ISRUC format (v2-keymodified)
                        value = {
                            'sample': seg['signal'],  # Changed from 'signal' to 'sample'
                            'label': seg['label'],
                            'data_info': {  # Changed from 'metadata' to 'data_info'
                                ...
                                'resampling_rate': TARGET_SR,
                                'original_sampling_rate': ORIGINAL_SR,
                                'segment_index': seg['original_index'],
                                'start_time': seg['original_index'] / ORIGINAL_SR,
                                'channel_names': elc_info['channel_names'],
                                'electrode_pairs': elc_info['electrode_pairs'],
                                'electrode_positions': elc_info['electrode_positions'],
