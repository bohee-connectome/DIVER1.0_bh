#!/usr/bin/env python3
"""
CHB-MIT LMDB Database Verification Script (Single LMDB version)
                ...
                try:
                    ...
                    for req_key in required_keys:
                        if req_key not in data:
                            errors.append(f"{split}: Missing key '{req_key}': {key_str}")
                    ...
                    if 'metadata' in data:
                        ...
                        if 'split' in metadata and metadata['split'] != split:
                            errors.append(f"{split}: Split mismatch - metadata says {metadata['split']}: {key_str}")
