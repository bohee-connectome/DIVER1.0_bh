#!/usr/bin/env python3
"""
Downsample CHBMIT LMDB from 500Hz to 200Hz

Reads existing 500Hz LMDB and creates new 200Hz version.
Only changes: sample shape (16,10,500)→(16,10,200) and resampling_rate 500→200
...
def downsample_signal(signal_500hz, target_samples=200):
    """Downsample from (16, 10, 500) to (16, 10, 200)"""
...
def check_lmdb_structure(lmdb_path, num_samples=3):
    ...
    with env.begin() as txn:
        ...
        for key, value in cursor:
            ...
            print(f"")
            print(f"WILL CHANGE:")
            print(f"  sample.shape:  {data['sample'].shape} → (16, 10, 200)")
            ...
            for key_name in data['data_info']:
                if key_name != 'resampling_rate':
                    val = data['data_info'][key_name]
                    if isinstance(val, (dict, np.ndarray)) and len(str(val)) > 50:
                        print(f"    {key_name}: <preserved>")
                    else:
    ...
    print("  1. sample shape: (16, 10, 500) → (16, 10, 200)")
...
def downsample_lmdb(input_lmdb_path, output_lmdb_path, batch_size=1000):
    ...
    print(f"Shape: (16, 10, 500) → (16, 10, 200)\n")
    ...
    with input_env.begin() as input_txn:
        ...
        try:
            for key, value in tqdm(cursor, total=total_samples, desc="Downsampling"):
                ...
                # Downsample signal
                signal_500 = data['sample']  # (16, 10, 500)
...
def main():
    parser = argparse.ArgumentParser(description="Downsample CHBMIT LMDB 500Hz → 200Hz")
